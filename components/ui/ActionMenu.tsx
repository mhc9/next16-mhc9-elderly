        ...
        <div className="relative menu-container flex justify-end">
            <button
                ...
                className={`p-2 rounded-xl transition-all shadow-sm cursor-pointer hover:scale-105 active:scale-95 ${
                    isOpen ? "bg-primary text-white" : "bg-muted hover:bg-primary hover:text-white"
                }`}
            >
                <MoreVertical size={18} />
            </button>
            ...
            {isOpen && menuPosition && createPortal(
                ...
                >
                    {items.map((item, index) => {
                        const baseClasses = "flex items-center gap-3 px-4 py-2.5 text-sm font-bold transition-colors w-full text-left";
                        const borderClass = index > 0 ? "border-t border-border/50" : "";
                        const variantClass = item.variant === 'danger' ? "text-rose-600 hover:bg-rose-50" : "text-foreground hover:bg-muted";
